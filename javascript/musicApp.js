	...
	this.showAlbum = function(data) {
		...
		if(data.resultCount > 0) {
			for(var i = 0; i < data.results.length; i++) {
				...
				template += '<span class="image bg" style="background-image: url(' + data.results[i].artworkUrl100 + ');"></span>';
				template += '<span class="album">' + data.results[i].collectionName + '</span>';
				template += '<span class="price">Price: USD ' + data.results[i].collectionPrice + '</span>';
				template += '<a class="buy-link" target="_blank" href="' + data.results[i].collectionViewUrl + '"><strong>Buy now</strong></a>';
				template += '</li>';
			}
			...
			noResultContainer.insertAdjacentHTML('afterbegin', '<h2 class="title">No results founded, try again.</h2>');
		}
	};
